...
3-offload~\cite{zero-offload} on 2 and 4 GPUs for completeness. We do not compare the results of ZeRO 
against the other three distributed training strategies.
...
The model uses identical hyperparameters to the Megatron experiments (Table \ref{tab:model_confs}) but 
implements standard PyTorch parameter management instead of Megatron's custom parameter sharding, enabling 
ZeRO compatibility. The configuration differences between ZeRO and the Megatron experiments are only in 
the batch size and sequence lengths used, due to ZeRO's microbatching strategy, see \ref{tab:zero_confs}. 
The optimizer used for the ZeRO experiments was once again Adam, with a learning rate of \texttt{2e-4}.
...
it ends. Given the timing strategy described above, we can track the wall-clock time of the 200 
iterations. With the number of training steps, batch size, and sequence length, we know the 
total number of tokens a strategy will see. This is the basis of our throughput computation.
...
necessary given how drastically differently Megatron prefers to operate compared to PyTorch. Following 
NVIDIA's example, the first thing any Megatron driven experiment does is tear down any previous parallel
states. Our experiments only ever run one type of Megatron model-parallel grouping, so this is more a 
matter of correctness and defensiveness. Next, we initialize the distributed process group to use the
NCCL backend. Now, with each device aware of the world size and their respective rank, the specific 
model-parallel grouping can be initialized for the particular Megatron parallelism (Table 
\ref{tab:megatron_strat_conf}). By setting tensor or pipeline parallel size to 1 effectively disables it 
from the experiment.
...
\subsubsection{Data Parallelism}\label{sec:3.a.2-dp}
...
For data parallelism, Megatron is configured with both tensor parallel and pipeline parallel sizes set to 1.
(Table \ref{tab:megatron_strat_conf}).
...
\textit{gradient\_accumulation\_steps} (GAS) to 4, which produced per-GPU microbatch sizes of 2 (2 GPUs) and 1 (4 
GPUs). This adjustment also required maintaining a separate set of ZeRO configurations from the one's used 
with in our Megatron models. Note that ZeRO's "microbatches" are unrelated to pipeline parallelism's 
microbatches.
...
communication of the all-reduce until the optimizer step.
...
\subsection{Metric Aggregations}
...
First we compute the "relative runtime overhead." This tell us how much slower or faster the multi-GPU run 
is compared to single GPU.
...
Our survey does not perform hyperparameter or kernel-level performance tuning. Instead, we focus 
exclusively on measuring the scaling behavior of established parallelization strategies under controlled 
conditions. All model, optimizer, and training parameters were held constant across experiments to ensure 
that differences in throughput and memory usage arise solely from the distributed training strategy rather 
...
minimizes the amount of communication needed. See Appendix \ref{app:mtron-pp-side-quest}.
...
\section{Megatron Throughput Plots}\label{app:mtron-throughput}
...
\section{Megatron Throughput Efficiency Plots}
...
\section{Megatron Peak Memory Usage Plots}
...
\section{Megatron Pipeline Parallelism Analysis Plots}\label{app:mtron-pp-side-quest}
...
\section{ZeRO Throughput Plots}\label{app:zero-throughput}
...
\section{ZeRO Throughput Efficiency Plots}
...
\section{ZeRO Peak Memory Usage}\label{app:zero-peak-mem}
...
\section{ZeRO Average Memory Usage}\label{app:zero-avg-mem}
