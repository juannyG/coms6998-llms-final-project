...
class MegatronSyntheticDataset(SyntheticDataset):
    def __getitem__(self, idx):
        ...
        return {
            ...
            #'position_ids': torch.arange(self.seq_len, dtype=torch.long).unsqueeze(0).expand(tokens.size(0), -1),  # Shape: [batch, seq_len]
            'position_ids': torch.arange(self.seq_len, dtype=torch.long),
            'labels': tokens.clone(), # We need something...so just use the tokens...
            'loss_mask': torch.ones_like(tokens, dtype=torch.float)  # Match tokens shape
            #'loss_mask': torch.ones(self.seq_len, dtype=torch.float)
        }

def get_iterator(conf, world_size):
    # Ensure rank 0 sets up first, then barrier
    if torch.distributed.get_rank() == 0:
        print("Rank 0: Setting up data iterator")
...
# Then in your forward_step_func, simplify it back to the original approach:
def forward_step_func(data_iterator, gpt_model):
    def loss_func(labels, output_tensor):
        current_rank = int(os.environ["LOCAL_RANK"]) 
        world_size = int(os.environ["WORLD_SIZE"])
        if current_rank == world_size - 1:  # Last rank computes loss
    ...
    # All ranks try to get data - let Megatron handle the coordination
    data = next(data_iterator)
    tokens = data["tokens"].to(device).transpose(0, 1)  # [seq, batch]
    position_ids = data["position_ids"].to(device).transpose(0, 1)
    labels = data["labels"].to(device)
    
    if current_rank == 0:  # Only rank 0 logs
        print(f"Rank {current_rank}: Input tokens shape: {tokens.shape}")
        print(f"Rank {current_rank}: Input position_ids shape: {position_ids.shape}")

    with record_function(MODEL_FORWARD_PROFILER_LABEL):
        output_tensor = gpt_model(tokens, position_ids, attention_mask=None)

    if current_rank == 0:
        print(f"Rank {current_rank}: Output shape: {output_tensor.shape}")
...
def run_tensor_parallel_experiment(_, conf, device, logger):
    ...
    * https://docs.nvidia.com/megatron-core/developer-guide/latest/user-guide/index.html#write-your-first-training-loop
    ...
    try:
        ...
        it = get_iterator(conf, world_size)
        #it = iter(loader)
        ...
        print(f"Rank {rank}: All ranks ready, starting training")
